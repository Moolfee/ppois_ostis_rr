...
GraphFromScResult BuildGraphFromSc(ScMemoryContext & ctx, ScAddr const & graphAddr)
{
  ...
  auto collectDistricts = [&](ScType const arcType)
  {
    ...
    while (it->Next())
    {
      ...
      // elem <- concept_district ? (допускаем const/var дугу)
  ...
  // Собираем районы, если они подключены const-дугой; если нет — пробуем var-дугу
  ...
  if (n == 0)
  {
    SC_LOG_WARNING("BuildGraphFromSc: graph has no districts (check concept_district arcs)");
    return result;
  }
  ...
  for (ScAddr const & route : routes)
  {
    ...
    // --- Вариант 1: через множество districtSet (route => nrel_connects_districts: {a; b;} )
    {
      ScAddr districtSet;
      ...
      auto tryFindSet = [&](ScType arcTypeRole) -> bool
      {
        ScIterator5Ptr it = ctx.CreateIterator5(
            ...
            ScType::Node,  // множество может быть любым типом узла
            arcTypeRole,
      ...
      {
        // не нашли множество — попробуем прямые дуги ниже
      }
      else
      {
    ...
    // --- Вариант 2: прямые дуги route => nrel_connects_districts: districtA; districtB;;
    auto collectDirect = [&](ScType arcCommonType, ScType arcRoleType)
    {
      ScIterator5Ptr it = ctx.CreateIterator5(
          route,
          arcCommonType,
          ScType::Node,   // район как узел
          arcRoleType,
          TransportAccessibilityKeynodes::nrel_connects_districts);
      ...
      while (it->Next())
      {
        ...
        // Проверяем, что это действительно район
        ScIterator3Ptr itDistrictClassConst = ctx.CreateIterator3(
            TransportAccessibilityKeynodes::concept_district,
            ScType::ConstPermPosArc,
            candidate);
    ...
    // --- Итог: берём только если нашли ровно 2 района
